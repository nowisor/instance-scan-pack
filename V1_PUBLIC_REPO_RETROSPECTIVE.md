...
A second `git filter-repo --message-callback` pass scrubbed instance-specific identifiers from sprint-era commit messages — substituting role-based names (`test-instance-A`, `test-instance-B`, `test-instance-C`) for the PDI URLs, generalizing specific timing and finding-count values to qualitative descriptions. The matching scrub was applied to the two QA artifact files (`kb-authoring/qa/bundle-test-results.md` and `kb-authoring/qa/pdi-install-verification.md`) on top of the filter-repo work. The older retrospective files (`V0_2_RETROSPECTIVE.md`, `V1_RETROSPECTIVE.md`, `V1_RETROSPECTIVE_ADDENDUM.md`, `V1_RETROSPECTIVE_TIER2.md`, `PILOT_RETROSPECTIVE.md`) were left in their original form — they document the build trajectory and the maintainer chose to preserve them as-is.
